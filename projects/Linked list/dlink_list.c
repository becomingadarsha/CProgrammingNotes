/*
     ...
     This file is 93,473 bytes long and contains 3,770 lines.
...
static int error_code, free_size, list_size, using_circular;
...
The variable using_circular has an invalid value: %d.\n\n",
     ...
     {
          error_code = success;
          ret = snprintf( str, LENGTH, "The %slist is empty.\n\n",
                          str_ptr );
          write_output( str );
          if ( list_size != 0 )
          {
...
The list size should be 0, but instead it seems to be %d.\n\n",
     ...
     if ( list_size == 1 )
     {
          ret = snprintf( str, LENGTH, "The %slist is using 1 node:\n\n",
                          str_ptr );
     }
     else
     {
          ret = snprintf( str, LENGTH, "The %slist is using %d nodes:\n\n",
                          str_ptr, list_size );
     }
     write_output( str );
...
&node %*d: %016X Node number: %*d. Next: %016X Prev: %016X\n",
     ...
     ret = snprintf( str, LENGTH, "Pointer last_list_node: %08X\n\n",
                     last_list_node );

#else  /* _64BITS */

     ret = snprintf( str, LENGTH, "Pointer last_list_node: %016X\n\n",
                     last_list_node );
...
Node %d has a bad magic number: %04X.  Exiting list_free_nodes().\n\n",
     ...
     do
     {
          ...
          ret = snprintf( str, LENGTH,
                          ...
                          space, count, ptr, ptr->next, ptr->prev );
     ...
     ret = snprintf( str, LENGTH, "Pointer last_free_node: %08X\n\n",
                     last_free_node );

#else  /* _64BITS */

     ret = snprintf( str, LENGTH, "Pointer last_free_node: %016X\n\n",
                     last_free_node );
...
void display_node_data( node_t *ptr )
{
     ...
     ret = snprintf( str, LENGTH, "Node address: %08X\n", ptr );

#else  /* _64BITS */

     ret = snprintf( str, LENGTH, "Node address: %016X\n", ptr );
     ...
     write_output( str );
     ret = snprintf( str, LENGTH, "node->pad:   %d\n", ( int )ptr->pad );
     write_output( str );

#endif

     ret = snprintf( str, LENGTH, "node->num:   %d\n", ptr->num );
     write_output( str );

#ifdef _32BITS

     ret = snprintf( str, LENGTH, "node->next:  %08X\n", ptr->next );

#else  /* _64BITS */

     ret = snprintf( str, LENGTH, "node->next:  %016X\n", ptr->next );
     ...
     ret = snprintf( str, LENGTH, "node->prev:  %08X\n\n", ptr->prev );

#else  /* _64BITS */

     ret = snprintf( str, LENGTH, "node->prev:  %016X\n\n", ptr->prev );
...
(%d) free_nodes->prev is not NULL when it should be.\n", count );
          ...
          last_free_node->magic == NODE_MAGIC_NUMBER )
     {
          count++;
          ret = snprintf( str, LENGTH, "\
(%d) The node last_free_node has a bad magic number.\n", count );
          write_output( str );
     }
